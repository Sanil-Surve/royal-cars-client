...
import { Calendar, CreditCard, FileText, ChevronRight } from "lucide-react";
    ...
    <div>
      ...
      >
        {items.map((b) => (
          ...
          >
            <Link to={`/bookings/${b.id}`} className="block" data-testid={`booking-link-${b.id}`}>
              <Card className="grid gap-4 rounded-lg border-slate-200 bg-white p-4 transition-shadow hover:shadow-md md:grid-cols-[140px_1fr_auto]" data-testid={`booking-${b.id}`}>
                ...
                <div className="flex flex-col items-end justify-between gap-2">
                  ...
                  {payRemaining && b.balance_amount > 0 && b.paid_amount > 0 && b.status === "confirmed" && (
                    <Button size="sm" onClick={(e) => { e.preventDefault(); payRemaining(b); }} variant="outline" className="rounded-md border-[#D4AF37] text-[#0A192F]" data-testid={`pay-balance-${b.id}`}>
