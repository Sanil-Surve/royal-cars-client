...
export default function CustomerDashboard() {
  ...
  const payRemaining = async (booking) => {
    try {
      await api.post("/payments/init", { booking_id: booking.id, payment_type: "full" });
      const mockId = `pay_mock_${Date.now()}`;
      await api.post("/payments/verify", { booking_id: booking.id, payment_id: mockId });
      toast.success("Remaining balance paid");
      load();
    } catch (e) {
  ...
  const payForBooking = async (booking, type) => {
    try {
      await api.post("/payments/init", { booking_id: booking.id, payment_type: type });
      const mockId = `pay_mock_${Date.now()}`;
      await api.post("/payments/verify", { booking_id: booking.id, payment_id: mockId });
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        {loading ? (
          ...
          <div className="space-y-10">
            <Section title="Active bookings" items={active} payForBooking={payForBooking} payRemaining={payRemaining} user={user} />
