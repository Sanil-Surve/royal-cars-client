            ...
            {bookings.map((b) => (
              ...
              >
                <TableCell>
                  ...
                  <div className="text-xs text-slate-500 hidden md:block">{b.customer_email}</div>
                </TableCell>
                <TableCell>{b.vehicle_name}</TableCell>
                <TableCell className="hidden md:table-cell text-xs text-slate-600">{b.pickup_date} → {b.dropoff_date}</TableCell>
                <TableCell className="hidden md:table-cell">{formatINR(b.total_amount)}</TableCell>
                <TableCell className="hidden md:table-cell">{formatINR(b.balance_amount || 0)}</TableCell>
                <TableCell className="hidden md:table-cell"><StatusBadge status={b.status} /></TableCell>
                <TableCell className="hidden md:table-cell text-right" onClick={(e) => e.stopPropagation()}>
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {b.status === "confirmed" && (
                      <Button
                        size="sm"
                        onClick={(e) => { e.stopPropagation(); setStartDialog({ open: true, booking: b }); }}
                    ...
                    {b.status === "active" && (
                      <Button
                        size="sm"
                        onClick={(e) => { e.stopPropagation(); setEndDialog({ open: true, booking: b }); }}
                    ...
                    <Select value={b.status} onValueChange={(v) => setStatus(b.id, v)}>
                      <SelectTrigger className="h-8 w-36 rounded-md" data-testid={`admin-booking-status-${b.id}`} onClick={(e) => e.stopPropagation()}><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {STATUSES.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                      </SelectContent>
                    </Select>
